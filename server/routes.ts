...
export async function registerRoutes(
  ...
  // Seed Data - MUST complete before routes are available
  await seedDatabase();
...
async function seedDatabase() {
  try {
    ...
    console.log(`✅ Database seeding completed. Total products: ${finalProducts.length}`);
  } catch (err) {
    console.error("❌ Database seeding failed:", err);
    throw err; // Re-throw to prevent server from starting with seeding errors
  }
}
